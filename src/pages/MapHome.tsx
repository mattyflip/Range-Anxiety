...
import { doc, getDoc, collection, addDoc, serverTimestamp, onSnapshot, query, where, deleteDoc, getDocs, updateDoc, arrayUnion } from 'firebase/firestore'
...
function MapHome() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        setUserData(null); setIsPro(false); setIsHostTier(false); setUsername('');
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snap) => {
      ...
      snap.forEach(docSnap => rides.push({ id: docSnap.id, ...docSnap.data() } as GroupRide));
    ...
    const rideUnsub = onSnapshot(doc(db, "group_rides", activeRide.id), (snap) => {
      if (snap.exists()) {
        ...
        if (data.status === 'offline') {
          alert(`The ride ${activeRide.name} has ended.`);
  ...
  useEffect(() => {
    ...
    if (savedRoute) {
      try {
        ...
        setIsRoundTrip(data.isRoundTrip || false); setIsCustomReturn(data.isCustomReturn || false);
  ...
  const saveCurrentBike = async () => {
    ...
    setNewBikeName('');
    alert("Bike saved to your library!");
  };
  ...
  const searchPOIs = async (category: string) => {
    ...
    if (category === 'charging' && !isPro && !isHostTier) { handleUpgrade('pro'); return; }
  ...
  const calculateMetrics = async (result: google.maps.DirectionsResult, routeIndex: number = 0) => {
    try {
      ...
      let totalDistMeters = 0; route.legs.forEach(leg => totalDistMeters += (leg.distance?.value || 0));
      ...
      const MotorPowerWatts = Math.max(0, (ForceRolling + ForceDrag) * velocityMps - (controlType === 'pas' ? 100 : 0));
      ...
      setMetrics({ distanceMiles: distMiles, durationMin: distMiles / (Number(targetSpeedMph) || 15) * 60, elevationGainFeet: gainFeet, elevationLossFeet: lossFeet, estimatedWh, batteryPercentUsed: Math.max(0, batteryPercentRemaining), recommendedSpeedMph: 20, deathPoint, windConditions: { speed: windSpeed, direction: windDir, headwindComponent: headwindMph } });
      setIsLoading(false);
  ...
  const shareToCommunity = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "posts"), { authorId: user.uid, authorUsername: userData?.username || 'Rider', authorProfilePic: userData?.profilePic || '', imageUrl: url, caption: `Rode ${metrics.distanceMiles.toFixed(1)} miles!`, likes: [], commentsEnabled: true, createdAt: serverTimestamp(), city: userData?.city || '', homeRegion: userData?.homeRegion || '', tripData: { origin: trip.origin, destination: trip.destination, waypoints: trip.waypoints, isRoundTrip } });
      alert("Shared!"); setIsLoading(false);
    } catch (e) { setIsLoading(false); }
  };
      ...
      <div className="main-layout">
        <aside className={`sidebar ${showMobileMenu ? 'mobile-visible' : ''}`}>
          ...
          <section className="form-group" style={{ position: 'relative' }}>
            ...
            <input type="text" placeholder="Search models..." value={bikeSearchQuery} onFocus={() => setShowBikeResults(true)} onChange={e => setBikeSearchQuery(e.target.value)} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            ...
            <section className="form-group"><label>Capacity (Ah/Wh)</label><input type="number" value={specs.capacityAh} onChange={e => handleSpecChange('capacityAh', e.target.value)} /></section>
          </div>
          ...
          {metrics && (
            <div ref={metricsCardRef} className="card metrics-card" style={{ marginTop: '2rem', borderLeft: '4px solid #ff6600', padding: '1.5rem', background: 'rgba(40,40,40,0.9)' }}>
              {metrics.batteryPercentUsed <= 0 && <div style={{ color: '#ff4444', marginBottom: '1rem', fontSize: '0.8rem' }}>☠️ Battery will die! Check map.</div>}
              <div style={{ fontSize: '2.5rem', fontWeight: 900 }}>{metrics.batteryPercentUsed.toFixed(1)}% Left</div>
              <p style={{ margin: '0.5rem 0', color: '#888' }}>{metrics.distanceMiles.toFixed(1)} mi • {Math.round(metrics.durationMin)} min</p>
              <button onClick={() => setShowSharePreview(true)} style={{ width: '100%', marginTop: '1rem', padding: '1rem', background: '#ff6600', border: 'none', borderRadius: '12px', color: 'white', fontWeight: 'bold', cursor: 'pointer' }}>Share Trip</button>
            </div>
          )}
          ...
          {isLoaded ? (
            <GoogleMap mapContainerStyle={{ width: '100%', height: '100%' }} center={center} zoom={10} onLoad={onMapLoad}>
              {trip.origin && trip.destination && isLoading && !response && (
                <DirectionsService 
                  options={{ origin: trip.origin, destination: isRoundTrip ? trip.origin : trip.destination, travelMode: google.maps.TravelMode.BICYCLING }} 
      ...
      <div className="persistent-controls" style={{ position: 'fixed', bottom: '2rem', left: '50%', transform: 'translateX(-50%)', zIndex: 2000, display: 'flex', gap: '1rem', background: 'rgba(20,20,20,0.9)', padding: '0.8rem 1.5rem', borderRadius: '40px', border: '1px solid #333' }}>
        ...
        >
          {showMobileMenu 
            ...
            : (metrics && !settingsDirty ? '📊 Trip Stats' : '🏁 Start Here')
          }
        </button>
